...
document.getElementById('memberOut').addEventListener('click', function() {
  ...
  $.ajax({
    type: "POST",
    url: "/manager/replyDelete",
    contentType: "application/json",
    data: JSON.stringify({ replyNumber: selectedUserNumbers }), // 배열을 JSON으로 변환
    success: function(response) {
      alert("선택된 댓글이 삭제 처리되었습니다.");
      window.location.href = "/manager/manageComment"; // 리다이렉션
    },
    error: function() {
      alert("삭제 처리에 실패했습니다.");
    }
