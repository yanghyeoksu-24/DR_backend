...
document.getElementById('memberOut').addEventListener('click', function() {
    ...
    $.ajax({
        type: "DELETE",
        url: "/manager/recipeDelete",
        contentType: "application/json",
        data: JSON.stringify({ recipeNumber: selectedBoardNumbers }), // 배열을 JSON으로 변환
        success: function(response) {
            alert("선택된 레시피가 삭제 처리되었습니다.");
            window.location.href = "/manager/manageRecipe"; // 리다이렉션
        },
        error: function() {
            alert("삭제 처리에 실패했습니다.");
        }
